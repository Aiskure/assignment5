...
def generate_outputs(llm, examples, prompt_format="zero_shot"):
    if prompt_format == "alpaca":
        prompts = [ALPACA_TEMPLATE.format(instruction=ex["prompt"]) for ex in examples]
        stop_seqs = ["### Instruction:", "\n\n\n"]
    else:
        prompts = [system_template.format(instruction=ex["prompt"]) for ex in examples]
        stop_seqs = ["# Query:"]
...
def parse_args():
    parser = argparse.ArgumentParser(description="SimpleSafetyTests output generation")
    parser.add_argument("--model_path", type=str, default=None)
    parser.add_argument("--prompt_format", type=str, default="zero_shot",
                        choices=["zero_shot", "alpaca"])
    parser.add_argument("--data_path", type=str,
                        default="data/SimpleSafetyTests/simple_safety_tests.jsonl")
...
if __name__ == "__main__":
    ...
    if args.output_path:
        ...
        output_path = f"outputs/sst/{args.prompt_format}_model_outputs.jsonl"
    ...
    examples = load_sst(args.data_path)
    results, timing = generate_outputs(llm, examples, args.prompt_format)
